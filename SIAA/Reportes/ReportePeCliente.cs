...
namespace SIAAPI.Reportes
{
    public class ReportePeCliente
    {
        ...
        List<ClientesPe> clientes = new List<ClientesPe>();
        #endregion
        public ClientesPeContext db = new ClientesPeContext();
        // aqui los puedes pasar como parametro a l reporte

        public byte[] PrepareReport(DateTime _fechaini, DateTime _fechafin, int idcliente)

        {
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"PedidosClienteDls.pdf" + "\"");
        ...
        public List<Pedidos> GetRep(int idcliente)
        {
            List<Pedidos> rep = new List<Pedidos>();
            string cadena = "";
            try
            {
                if(idcliente == 0)
                {
                    cadena = "SELECT P.idPedido, p.fecha,p.subtotal, p.iva, p.total, p.IDMoneda, CAST(1 AS DECIMAL(8,2)) as TC, p.status, CAST(0 AS DECIMAL(8,2)) as TotalDls FROM clientes AS c INNER JOIN encPedido AS p ON c.IDCliente=p.idcliente where fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59'";
                }
                else
                {
                ...
                rep = db.Database.SqlQuery<Pedidos>(cadena).ToList();
        ...
        public List<ClientesPe> GetClientes(int idCliente)
        {
            List<ClientesPe> clientes = new List<ClientesPe>();
            string cadena = "";
            try
            {
                if (idCliente == 0)
                {
                    cadena = "SELECT  distinct c.IDCliente, c.nombre as Nombre, c.Telefono , c.IDVendedor from clientes as c inner join encPedido as p on c.idcliente=p.idcliente where c.status='activo' and fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59' order by Nombre";
                }
                else
                {
                    cadena = "SELECT  distinct c.IDCliente, c.nombre as Nombre, c.Telefono , c.IDVendedor from clientes as c inner join encPedido as p on c.idcliente=p.idcliente where c.status='activo' and fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59' and p.IdCliente=" + idCliente + " order by Nombre";

                }
                    clientes = db.Database.SqlQuery<ClientesPe>(cadena).ToList();
        ...
        public List<detped> Getdetallepedido(int idPedido)
        {
            List<detped> detpe = new List<detped>();
            ...
            detpe = db.Database.SqlQuery<detped>(cadena).ToList();
        ...
        private void ReportHeader()
        {
            ...
            _PdfPCell = new PdfPCell(new Phrase("Pedidos Cliente en Dolares", _fontStyle));
            ...
            foreach (ClientesPe cliente in clientes)
            {
                float[] anchoColumnasCliente = { 300f, 300f };

                PdfPTable tablacliente = new PdfPTable(anchoColumnasCliente);
                //tablaProductosPrincipal.DefaultCell.Border = Rectangle.NO_BORDER;

                tablacliente.SetTotalWidth(anchoColumnasCliente);
                tablacliente.SpacingBefore = 0;
                tablacliente.HorizontalAlignment = Element.ALIGN_LEFT;
                tablacliente.LockedWidth = true;
                Font _fontStyleCliente = new Font(Font.FontFamily.TIMES_ROMAN, 9, Font.BOLD);  // aqui podremos cambiar la fuente de LA FECHA y su tamanño

                PdfPCell tablacliente1 = new PdfPCell(new Phrase(cliente.Nombre, _fontStyleCliente));
                tablacliente1.Border = 0;
                tablacliente1.FixedHeight = 20f;
                tablacliente1.HorizontalAlignment = Element.ALIGN_LEFT;
                tablacliente1.VerticalAlignment = Element.ALIGN_MIDDLE;
                tablacliente1.BackgroundColor = BaseColor.WHITE;
                tablacliente.AddCell(tablacliente1);


                PdfPCell tablacliente2 = new PdfPCell(new Phrase("Teléfono: "+cliente.Telefono, _fontStyleCliente));
                tablacliente2.Border = 0;
                tablacliente2.FixedHeight = 20f;
                tablacliente2.HorizontalAlignment = Element.ALIGN_LEFT;
                tablacliente2.VerticalAlignment = Element.ALIGN_MIDDLE;
                tablacliente2.BackgroundColor = BaseColor.WHITE;
                tablacliente.AddCell(tablacliente2);



                _documento.Add(tablacliente);
            }
        ...
        private void ReportBody()
        {
            ...
            foreach (ClientesPe cliente in clientes)
            {
                ...
                float[] anchoColumnasclientes = { 300f, 300f };
                ...
                //PdfPCell _PdfPCell2 = new PdfPCell(new Phrase("", _fontStylecliente));
                //_PdfPCell2.Border = 0;
                ...
                PdfPCell _PdfPCell4 = new PdfPCell(new Phrase("Vendedor: " + nombrevendedor, _fontStylecliente));
                _PdfPCell4.Border = 0;
                ...
                foreach (Pedidos pedido in pedidos)
                {
                    ...
                    List<detped> detpedidos = new List<detped>();
                    ...
                    foreach (detped dpedido in detpedidos)
                    {
                        // creamos una tabla para imprimir los los datos del cliente
                        // como son 4 columnas a imprimir 600entre las 4 
    ...
    public class ClientesPeContext : DbContext
    {
        public ClientesPeContext() : base("name=DefaultConnection")
        {
            Database.SetInitializer<ClientesPeContext>(null);
        }
