...
{
    public class ReportePedidosDlsVen
    {
        #region Declaration
        ...
        public VendedorPedidoDlsVenContext db = new VendedorPedidoDlsVenContext();
...

        {
            ...
            vendedores = this.GetPedidoVendedorDls();
          ...
          //  _pdfTable.HeaderRows = 4;
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"PedidoVendedorEnDolares.pdf" + "\"");
  ...
  //Obtener la lista de vendedores en un periodo de fechas
        public List<VendedorPedidoDls> GetPedidoVendedorDls()
        {
            List<VendedorPedidoDls> vendedores = new List<VendedorPedidoDls>();
            try
            {
                string cadena = "select  distinct E.IDVendedor, V.Nombre, V.CuotaVendedor, M.ClaveMoneda from [dbo].[EncPedido] as E inner join [dbo].[Vendedor] as V on E.IDVendedor = V.IDVendedor inner join [dbo].[c_Moneda] as M on V.IDMoneda = M.IDMoneda where fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59' and e.status<>'cancelado' order by v.nombre";
        ...
        public List<ClientesPedidoDls> GetPedidoClientesDls(int idvendedor)
        {
            ...
            try {
                ...
                clientes = db.Database.SqlQuery<ClientesPedidoDls>(cadena).ToList();
            }catch(SqlException err)
            {
                string mensajedeerror = err.Message;
            }
        ...
        public List<ReportePedidoDlsVen> GetPedidoPedidoDls(int idcliente, int idvendedor)
        {
            List<ReportePedidoDlsVen> pedido = new List<ReportePedidoDlsVen>();
            try
            {
                ...
                pedido = db.Database.SqlQuery<ReportePedidoDlsVen>(cadena).ToList();
            }
            catch (SqlException err)
            {
                string mensajedeerror = err.Message;
        ...
        private void ReportBody()
        {
            ...
            foreach (VendedorPedidoDls vendedor in vendedores)
            {
                ...
                List<ClientesPedidoDls> clientes = new List<ClientesPedidoDls>();
                  clientes = GetPedidoClientesDls(vendedor.IDVendedor);
                foreach (ClientesPedidoDls cliente in clientes)
                {
                    ...
                    pedidos = GetPedidoPedidoDls(cliente.IDCliente, vendedor.IDVendedor); // aqui obtenemos los pedidos que corresponden a ese cliente
                    ...
                    foreach (ReportePedidoDlsVen pedido in pedidos)
                    {
                        ...
                        Font _fontStylepedido = new Font(Font.FontFamily.HELVETICA, 7, Font.NORMAL);  // aqui podremos cambiar la fuente de lcientes y su tamanño
                        ...
                        foreach (detpedidoReporte dpedido in detpedidos)
                        {
                            ...
                            tablaparaestedepedido.SpacingBefore = 1;
                            ...
                            Font _fontStyledep = new Font(Font.FontFamily.HELVETICA, 5, Font.NORMAL);  // aqui podremos cambiar la fuente de lcientes y su tamanño


                            PdfPCell _PdfPCell2dep = new PdfPCell(new Phrase("CREF: " + dpedido.Cref, _fontStyledep));
                            _PdfPCell2dep.Border = 0;
                            _PdfPCell2dep.SetLeading(1,1);
    ...
    public class ReportePedidoDlsVen
    {
        [Display(Name = "fecha")]
    ...
    public class VendedorPedidoDls
    {
        [Key]
    ...
    public class ClientesPedidoDls
    {
        [Key]
    ...
    public class detpedidoReporteDlsVen
    {
        [Key]
        public int IDPedido { get; set; }
    ...
    public class VendedorPedidoDlsVenContext : DbContext
    {
        public VendedorPedidoDlsVenContext() : base("name=DefaultConnection")
        {
            Database.SetInitializer<ClientesPedidoContext>(null);
        }
        public DbSet<ReportePedidoDlsVen> Pedidos { get; set; }
        public DbSet<ClientesPedidoDls> Clientes { get; set; }
        public DbSet<detpedidoReporteDlsVen> DetPedido { get; set; }
