...
namespace SIAAPI.Reportes
{
    public class ReporteRemisiones
    {
        ...
        List<ClienteRemision> clientes = new List<ClienteRemision>();
        #endregion
        public ClienteRemisionContext db = new ClienteRemisionContext();
...

        {
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"Remisiones.pdf" + "\"");
        ...
        public List<ReporteRemision> GetRep(int idcliente)
        {
            List<ReporteRemision> rep = new List<ReporteRemision>();
            try
            {
                string cadena = "SELECT re.idRemision, re.fecha,  re.subtotal, re.iva, re.total, re.IDMoneda, CAST(1 AS DECIMAL(8,2)) as TipoCambio, re.status, re.Total FROM clientes AS c INNER JOIN encRemision AS re ON c.IDCliente=re.idcliente where fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59'";
                rep = db.Database.SqlQuery<ReporteRemision>(cadena).ToList();
        ...
        public List<ClienteRemision> GetClientes()
        {
            List<ClienteRemision> clientes = new List<ClienteRemision>();
            try
            {
                string cadena = "SELECT c.IDCliente, c.nombre as Nombre, c.Telefono from clientes as c inner join encremision as co on c.idcliente=co.idcliente where c.status='activo' and fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59'";
                clientes = db.Database.SqlQuery<ClienteRemision>(cadena).ToList();
        ...
        public List<detRemision> GetdetRemi(int idRemision)
        {
            List<detRemision> detpe = new List<detRemision>();
            string cadena = "SELECT d.IDRemision, a.Cref, d.Cantidad, d.suministro, d.importe,  d.importeiva, d.importetotal from detRemision as d  inner join encRemision as re on d.IDRemision=re.IDRemision inner join articulo as a on d.idarticulo=a.idarticulo and re.IDRemision=" + idRemision + "";
            detpe = db.Database.SqlQuery<detRemision>(cadena).ToList();
        ...
        private void ReportBody()
        {
            ...
            foreach (ClienteRemision cl in clientes)
            {
                ...
                List<ReporteRemision> remisiones = new List<ReporteRemision>();

                remisiones = GetRep(cl.IDCliente); // aqui obtenemos los pedidos que corresponden a ese cliente
                ...
                foreach (ReporteRemision remi in remisiones)
                {
                    ...
                    string fecha = remi.fecha.ToShortDateString();
                    string idpedido = Convert.ToString(remi.IdRemision);
                    string subtotal = remi.subtotal.ToString("C"); // COn esto le indico que lo comverta a formato meneda y luego a string para que los millares a aparezcan separados por coma
                    string iva = remi.IVA.ToString("C");
                    string total = remi.total.ToString("C");
                    ...
                    PdfPCell _PdfPCell7pedido = new PdfPCell(new Phrase(remi.status, _fontStylepedido));
                    ...
                    PdfPCell _PdfPCell8pedido = new PdfPCell(new Phrase(remi.TipoCambio.ToString("C"), _fontStylepedido));
                    ...
                    List<detRemision> detremi = new List<detRemision>();

                    detremi = GetdetRemi(remi.IdRemision); // aqui obtenemos los pedidos que corresponden a ese cliente

                    // antes de entrar al foreach de los pedidos

                    // preparado los acumuladores para imprimir totales por cluentes si quisiera pero su quiero

                    foreach (detRemision dpcoti in detremi)
                    {
                        // creamos una tabla para imprimir los los datos del cliente
                        // como son 4 columnas a imprimir 600entre las 4 
    ...
    public class ClienteRemisionContext : DbContext
    {
        public ClienteRemisionContext() : base("name=DefaultConnection")
        {
            Database.SetInitializer<ClienteRemisionContext>(null);
        }
        public DbSet<ReporteRemision> Cotizacion { get; set; }
        public DbSet<ClienteRemision> Clientes { get; set; }
        public DbSet<detRemision> DetCotizacion { get; set; }
