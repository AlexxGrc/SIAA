...
namespace SIAAPI.Reportes
{
    public class ReporteCotizaciones
    {
        ...
        List<ClienteCotizacion> clientes = new List<ClienteCotizacion>();
        #endregion
        public ClienteCotizacionContext db = new ClienteCotizacionContext();
...

        {
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"Cotizaciones.pdf" + "\"");
        ...
        public List<ReporteCotizacion> GetRep(int idcliente)
        {
            List<ReporteCotizacion> rep = new List<ReporteCotizacion>();
            try
            {
                string cadena = "SELECT co.idCotizacion, co.fecha,  co.subtotal, co.iva, co.total, co.IDMoneda, CAST(1 AS DECIMAL(8,2)) as TipoCambio, co.status, co.Total FROM clientes AS c INNER JOIN enccotizacion AS co ON c.IDCliente=co.idcliente where fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59'";
                rep = db.Database.SqlQuery<ReporteCotizacion>(cadena).ToList();
        ...
        public List<ClienteCotizacion> GetClientes()
        {
            List<ClienteCotizacion> clientes = new List<ClienteCotizacion>();
            try
            {
                string cadena = "SELECT c.IDCliente, c.nombre as Nombre, c.Telefono from clientes as c inner join enccotizacion as co on c.idcliente=co.idcliente where c.status='activo' and fecha>='" + fechaini.Year + "/" + fechaini.Month + "/" + fechaini.Day + " 00:00:01' and fecha <='" + fechafin.Year + "/" + fechafin.Month + "/" + fechafin.Day + " 23:59:59'";
                clientes = db.Database.SqlQuery<ClienteCotizacion>(cadena).ToList();
        ...
        public List<detCotizacion> GetdetCoti(int idCotizacion)
        {
            List<detCotizacion> detpe = new List<detCotizacion>();
            string cadena = "SELECT d.IDCotizacion, a.Cref, d.Cantidad, d.suministro, d.importe,  d.importeiva, d.importetotal from detCotizacion as d  inner join encCotizacion as co on d.IDCotizacion=co.IDCotizacion inner join articulo as a on d.idarticulo=a.idarticulo and co.IDCotizacion=" + idCotizacion + "";
            detpe = db.Database.SqlQuery<detCotizacion>(cadena).ToList();
        ...
        private void ReportHeader()
        {
            ...
            _PdfPCell = new PdfPCell(new Phrase("Reporte de Remisiones", _fontStyle));
            ...
            _fontStyle = FontFactory.GetFont("Tahoma", 8f, 1);
            _PdfPCell = new PdfPCell(new Phrase("Remisión", _fontStyle));
            _PdfPCell.Border = 0;
            _PdfPCell.HorizontalAlignment = Element.ALIGN_LEFT;
            _PdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
            _PdfPCell.BackgroundColor = BaseColor.GRAY;
        ...
        private void ReportBody()
        {
            ...
            foreach (ClienteCotizacion cl in clientes)
            {
                ...
                PdfPCell _PdfPCell1 = new PdfPCell(new Phrase(cl.IDCliente.ToString(), _fontStylecliente));
                _PdfPCell1.Border = 0;
                ...
                PdfPCell _PdfPCell2 = new PdfPCell(new Phrase(cl.Nombre, _fontStylecliente));
                _PdfPCell2.Border = 0;
                ...
                PdfPCell _PdfPCell3 = new PdfPCell(new Phrase(cl.Telefono, _fontStylecliente));
                _PdfPCell3.Border = 0;
                ...
                //string Almacen = new AlmacenContext().Almacenes.Find(cliente.a).Nombre;
                ...
                List<ReporteCotizacion> cotizaciones = new List<ReporteCotizacion>();

                cotizaciones = GetRep(cl.IDCliente); // aqui obtenemos los pedidos que corresponden a ese cliente
                ...
                foreach (ReporteCotizacion coti in cotizaciones)
                {
                    ...
                    string fecha = coti.fecha.ToShortDateString();
                    string idpedido = Convert.ToString(coti.IdCotizacion);
                    string subtotal = coti.subtotal.ToString("C"); // COn esto le indico que lo comverta a formato meneda y luego a string para que los millares a aparezcan separados por coma
                    string iva = coti.IVA.ToString("C");
                    string total = coti.total.ToString("C");
                    ...
                    PdfPCell _PdfPCell6pedido = new PdfPCell(new Phrase("", _fontStylepedido));
                    ...
                    PdfPCell _PdfPCell7pedido = new PdfPCell(new Phrase(coti.status, _fontStylepedido));
                    ...
                    PdfPCell _PdfPCell8pedido = new PdfPCell(new Phrase(coti.TipoCambio.ToString("C"), _fontStylepedido));
                    ...
                    PdfPCell _PdfPCell9pedido = new PdfPCell(new Phrase("", _fontStylepedido));
                    ...
                    List<detCotizacion> detcoti = new List<detCotizacion>();

                    detcoti = GetdetCoti(coti.IdCotizacion); // aqui obtenemos los pedidos que corresponden a ese cliente

                    // antes de entrar al foreach de los pedidos

                    // preparado los acumuladores para imprimir totales por cluentes si quisiera pero su quiero

                    foreach (detCotizacion dpcoti in detcoti)
                    {
                        // creamos una tabla para imprimir los los datos del cliente
                        // como son 4 columnas a imprimir 600entre las 4 

                        float[] anchoColumnasdep = { 100f, 100f, 100f, 100f, 100f, 100f };
                        ...
                        PdfPCell _PdfPCell2dep = new PdfPCell(new Phrase("CREF: " + dpcoti.Cref, _fontStyledep));
                        ...
                        PdfPCell _PdfPCell1depp = new PdfPCell(new Phrase("Cantidad: " + dpcoti.cantidad.ToString(), _fontStyledep));
                        ...
                        PdfPCell _PdfPCell1dep = new PdfPCell(new Phrase("Suministro: " + dpcoti.suministro.ToString(), _fontStyledep));
                        ...
                        PdfPCell _PdfPCell3dep = new PdfPCell(new Phrase("Importe $ " + dpcoti.importe.ToString(), _fontStyledep));
                        ...
                        PdfPCell _PdfPCell4dep = new PdfPCell(new Phrase("Importe + IVA $ " + dpcoti.importeiva.ToString(), _fontStyledep));
                        ...
                        PdfPCell _PdfPCell5dep = new PdfPCell(new Phrase("Importe total $ " + dpcoti.importetotal.ToString(), _fontStyledep));
    ...
    public class ClienteCotizacionContext : DbContext
    {
        public ClienteCotizacionContext() : base("name=DefaultConnection")
        {
            Database.SetInitializer<ClienteCotizacionContext>(null);
        }
        public DbSet<ReporteCotizacion> Cotizacion { get; set; }
        public DbSet<ClienteCotizacion> Clientes { get; set; }
        public DbSet<detCotizacion> DetCotizacion { get; set; }
