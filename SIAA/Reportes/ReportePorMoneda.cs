...
namespace SIAAPI.Reportes
{
    public class ReportePorMoneda
    {
        ...
        MonedaReContext db = new MonedaReContext();
        ...
        public byte[] prepararReporte(int _IDMoneda)

        {
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"Reporte por moneda.pdf" + "\"");
        ...
        public List<MonedaRe> GetMoneda(int idMoneda)
        {
            List<MonedaRe> rep = new List<MonedaRe>();
            try
            {
                if (idMoneda != 0)
                {
                    string cadena = "  select distinct idarticulo, cref,descripcion,IDMoneda,IDFamilia,IDClaveUnidad,IDTipoArticulo, namefoto from Articulo where IDMoneda = "+idMoneda+" order by Descripcion";
                    rep = db.Database.SqlQuery<MonedaRe>(cadena).ToList();
                }
                else
                {
                    string cadena = "select distinct idarticulo, cref,descripcion,IDMoneda,IDFamilia,IDClaveUnidad,IDTipoArticulo, namefoto from Articulo order by IDMoneda, Descripcion";
                    rep = db.Database.SqlQuery<MonedaRe>(cadena).ToList();
                }
            }
        ...
        {
            _Titulo = "REPORTE GENERAL POR MONEDA";
        ...
        public List<c_Moneda> getTA(int idMoneda)
        {
            List<c_Moneda> TP = new List<c_Moneda>();
            try
            {
                if (idMoneda != 0)
                {
                    string cadena = "Select *from c_Moneda where IdMoneda=" + idMoneda;
                    TP = db.Database.SqlQuery<c_Moneda>(cadena).ToList();
                }
                else if (idMoneda == 0)
                {
                    string cadena = "Select *from c_Moneda";
                    TP = db.Database.SqlQuery<c_Moneda>(cadena).ToList();
                }
            }
            catch (SqlException err)
        ...
        private void cuerpoReporte()
        {
            ...
            TP = this.getTA(IDMoneda);

            foreach (c_Moneda TIP in TP)
            {
                ...
                List<MonedaRe> articulos = new List<MonedaRe>();//Lo usa el foreach
                articulos = this.GetMoneda(TIP.IDMoneda);
                foreach (MonedaRe articulo in articulos)
                {
    ...
    public class MonedaReContext : DbContext
    {
        public MonedaReContext() : base("name=DefaultConnection")

        {
            Database.SetInitializer<MonedaReContext>(null);
        }
