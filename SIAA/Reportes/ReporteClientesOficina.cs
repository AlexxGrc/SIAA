...
{
    public class ReporteClientesOficina
    {
        #region Declaration
        ...
        public List<Oficina> GetOficina(int idoficina)
        {
            List<Oficina> of = new List<Oficina>();
            // obtenemoslasofiinas             
            string cadena = "select * from oficina where IDOficina ="+idoficina;
        ...
        List<ReportefenoClie> facturas = new List<ReportefenoClie>();

        #endregion

        public byte[] PrepareReport(DateTime _fechaini, DateTime _fechafin, int _oficina, string _esta, string _nota)
        {
            fechaini = _fechaini;
            fechafin = _fechafin;
            idoficina = _oficina;
            ...
            //facturas = this.GetFacturas(int id);
            #region
            ...
            HttpContext.Current.Response.AddHeader("content-disposition", "attachment; filename=\"ReporteClientesOficina.pdf" + "\"");
        ...
        public List<ReportefenoClie> GetFacturas(int idofi)
        {
            List<ReportefenoClie> facturas = new List<ReportefenoClie>();
            ...
            try
            {
                if (status == "T")
                {
                    ...
                    cadena = cadena + "where FechaPago>='" + FI + "' and FechaPago <='" + FF + "'";
                        if (idofi != 0)
                    {
                        cadena = cadena + "and IDOficina = "+ idofi + "";
                    }
                  
                    facturas = db.Database.SqlQuery<ReportefenoClie>(cadena).ToList();
                }

                if (status == "A")
                {
                    string cadena = "select IDPagoFactura, FechaPago, IDCliente, Nombre, ClaveFormaPago, Descripcion, ClaveDivisa, TC, NoOperacion, Monto, RFCBancoEmisor,NombreBancoEmisor CuentaEmisor, RFCBancoReceptor, CuentaReceptor,  Clave as ClaveCadenaPago, TipoCadenaPago, CertificadoPago, IDTipoCadenaPago, SelloPago, Observacion, Estado from [dbo].[VPagoFacturaC]";
                    cadena = cadena + "where FechaPago>='" + FI + "' and FechaPago <='" + FF + "'  and Estado=1";
                    if (idofi != 0)
                    {
                        cadena = cadena + "and IDOficina = " + idofi + "";
                    }
                ...
                {
                    string cadena = "select IDPagoFactura, FechaPago, IDCliente, Nombre, ClaveFormaPago, Descripcion, ClaveDivisa, TC, NoOperacion, Monto, RFCBancoEmisor,NombreBancoEmisor CuentaEmisor, RFCBancoReceptor, CuentaReceptor,  Clave as ClaveCadenaPago, TipoCadenaPago, CertificadoPago, IDTipoCadenaPago, SelloPago, Observacion, Estado from [dbo].[VPagoFacturaC]";
                    cadena = cadena + "where FechaPago>='" + FI + "' and FechaPago <='" + FF + "' and Estado=0";
                    if (idofi != 0)
                    {
                        cadena = cadena + "and IDOficina = " + idofi + "";
                    }
        ...
        private void ReportHeader()
        {
            ...
            _PdfPCell = new PdfPCell(new Phrase("Reporte por Oficina", _fontStyle));
            ...
            foreach (Oficina oficina in ofi)
            {
                ...
                //List<Reportefeno> factu = new List<Reportefeno>();
                //factu = GetFacturas(oficina.IDOficina);//GetVendedorof
            ...
            //_fontStyle = FontFactory.GetFont("Tahoma", 8f, 1);
            _PdfPCell = new PdfPCell(new Phrase("No.Operacion", _fontStyle));
            _PdfPCell.Border = 0;
            _PdfPCell.HorizontalAlignment = Element.ALIGN_CENTER;
            _PdfPCell.VerticalAlignment = Element.ALIGN_MIDDLE;
            _PdfPCell.BackgroundColor = colordefinido;
        ...
        private void ReportBody()
        {
            ...
            foreach (Oficina oficina in ofi)
            {
                List<ReportefenoClie> factu = new List<ReportefenoClie>();
                factu = GetFacturas(oficina.IDOficina);//GetVendedorof

                foreach (ReportefenoClie fact in factu)
            {
                ...
                string fecha = fact.Pagada;//ToShortDateString()
                string ID = Convert.ToString(fact.IDCliente);
                ...
                PdfPCell _PdfPCell00pedido = new PdfPCell(new Phrase(fact.Nombre, _fontStyleparaestaFactura));
                ...
                PdfPCell _PdfPCell2pedido = new PdfPCell(new Phrase(fact.Descripcion, _fontStyleparaestaFactura));
                ...
                PdfPCell _PdfPCell6pedido = new PdfPCell(new Phrase(fact.TC.ToString(), _fontStyleparaestaFactura));
                ...
                PdfPCell _PdfPCell7pedido = new PdfPCell(new Phrase(fact.NoOperacion.ToString(), _fontStyleparaestaFactura));
                ...
                if (fact.ClaveDivisa == "USD" && fact.Estado == true)
                {
                    acumulasubtotalxUSD += fact.Monto;
                }
                if (fact.ClaveDivisa == "EUR" && fact.Estado == true)
                {
                    acumulasubtotalxEUR += fact.Monto;
                }
                if (fact.ClaveDivisa == "MXN" && fact.Estado == true)
                {
    ...
    public class ITextEventsRepLFacsss : PdfPageEventHelper
    {
    ...
    public class fechasReporteFacsss
    {
    ...
    public class ReporteFacturastss
    {
        [Key]
